...
\begin{proof}
\begin{itemize}
\item Since $f$ is a $k$-algebra homomorphism, $f$ must send $k$ to $k$; i.e., if $a\in k\subset k[x]$, then we must have $f(a)=\phi(a)$.
\item By assumption, we know $f(x)=r$.
\item Since $f$ preserves addition and multiplication, we have:
\begin{align*}
f(a_nx^n+a_{n_1}x^{n-2}+\cdots+a_0)=\phi(a_n)r^n+\phi(a_{n-1})r^{n-2}+\cdots +\phi(a_0)
\end{align}
\end{itemize}
